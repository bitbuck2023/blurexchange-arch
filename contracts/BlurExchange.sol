...
contract BlurExchange is IBlurExchange, ReentrancyGuarded, EIP712, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /* Constants */
    string public constant NAME = "BlurExchange";
        ...
        reentrancyGuard // move re-entrancy check for clarity
    {
        ...
        // 校验三个条件：订单不能是0地址，订单不能过期，订单没有成交或者取消
        ...
        returns (bool)
    {
        ...
        if (order.order.extraParams.length > 0 && order.order.extraParams[0] == 0x01) {
            console.log('validate oracle signature', 'enter oracle');
        ...
        console.log('validate oracle signature', 'not enter oracle');
    ...
    function _validateOracleAuthorization(
        ...
        if (signatureVersion == SignatureVersion.Single) {
            ...
            (v, r, s) = abi.decode(extraSignature, (uint8, bytes32, bytes32));
            */
            console.log("extraSignature v", v);
            console.logBytes32(r);
            console.logBytes32(s);
            console.logBytes(extraSignature);
    ...
    function testVerify(Input calldata sell, Input calldata buy) external view returns (bool) {
        // 给买方和卖方订单hash 基于EIP712
        bytes32 sellHash = _hashOrder(sell.order, nonces[sell.order.trader]);
        bytes32 buyHash = _hashOrder(buy.order, nonces[buy.order.trader]);
    ...
    // 校验服务器签名
    function testVerifyOracle(Input calldata input) external view returns (bool) {
        Order calldata order = input.order;
        console.log("order.trader", order.trader);
        // console.log("order.side", order.side);
        console.log("order.matchingPolicy", order.matchingPolicy);
        ...
        console.log("order.listingTime", order.listingTime);
        console.log("order.expirationTime", order.expirationTime);
        // console.log("order.fees", order.fees);
        console.log("order.salt", block.chainid);
        ...
        bytes memory noncebytes = abi.encode(0);

        console.log("orderbytes:");
        console.logBytes(orderbytes);
        console.log("noncebytes:");
        console.logBytes(noncebytes);
        
        bytes32 orderHash = _hashOrder(input.order, nonces[input.order.trader]);
        // 校验订单签名
        if (input.order.extraParams.length > 0 && input.order.extraParams[0] == 0x01) {
            console.log("testVerifyOracle", "start test");
            /* Check oracle authorization. */
    ...
    // 校验用户签名
    function testVerifyUser(Input calldata input) external view returns (bool) {
        bytes32 orderHash = _hashOrder(input.order, nonces[input.order.trader]);
        if (
            !_validateUserAuthorization(
                orderHash,
    ...
    function testExecute(Input calldata sell, Input calldata buy, uint8 step)
        external
        payable
        whenOpen
        setupExecution
    {
        _testexecute(sell, buy, step);
        _returnDust();
    }

    function _testexecute(Input calldata sell, Input calldata buy, uint8 step)
        public
        payable
        internalCall
        reentrancyGuard // move re-entrancy check for clarity
    {
        require(sell.order.side == Side.Sell, 'side must be sell');
